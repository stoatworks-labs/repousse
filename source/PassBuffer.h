...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    ...
    host's picture changes size. Nothing in these buffers outlives a frame
    (the ground is recomputed from the picture every time), so a
    reallocation that clears loses nothing -- see `cntest --resize`.
    ...
    **It owns its filtering.** Every buffer in this plugin is elevation
    read texel for texel with `texelFetch` -- the gradient is a central
    difference of exact neighbours, and a filtered read would put a texture
    unit's interpolation precision into every normal -- so every
    buffer here is Nearest. Linear and Mipmapped are tinsel's and are unused.

    Copied from tinsel (by way of rebate and standards), where it was written for a glow

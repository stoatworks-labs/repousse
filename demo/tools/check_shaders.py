...
`demo/plugin.js` holds five GLSL strings and so does `source/Shaders.cpp`.
That is two copies of the same text, and two copies drift -- quietly, because a
demo that renders a *plausible* sheet looks exactly like a demo that renders
...
Nothing else can. `rptest` drives the real plugin class through a real FFGL
sequence and has no idea this page exists, and `tools/check-shaders.sh`
compiles the C++ copies and never looks at the JS one.

------------------------------------------------------------------- what it does

Pulls each `R"( ... )"` string out of the C++ and each matching backtick
literal out of `plugin.js`, and compares them exactly -- no whitespace
normalisation, no comment stripping. This plugin's strings carry their own
`#version 410 core` line (there is no separate assemble step), so the literal
carries it too and the kit's `port()` is what swaps it for ES 3.00.
...
Nothing here checks the *ported* half. The pendulum, the unit conversions, the
blur weights and the pass order in plugin.js are a hand translation of
Pendulum.cpp, Controls.cpp and Repousse::ProcessOpenGL, and only a reader can
tell whether they still agree. When you change one of those, change it here
too -- and remember that a wrong port shows up on the page as a lamp that
swings at a plausible but wrong period, which nobody will notice.
"""
import os
import re
import sys
...
SHADERS = [
    ...
    ("HEIGHT", "source/Shaders.cpp", "kHeight"),
    ("BLUR", "source/Shaders.cpp", "kBlur"),
    ("MORPH", "source/Shaders.cpp", "kMorph"),
    ("SHADE", "source/Shaders.cpp", "kShade"),
]
...
def main():
    ...
    for name, path, symbol in SHADERS:
        ...
        if "${" in cpp_text:
            ...
            problems += 1
            continue
        if not cpp_text.startswith("#version 410 core\n"):
            print(f"FAIL  {symbol} does not open with the #version line the plugin hands the driver")
            problems += 1
            continue

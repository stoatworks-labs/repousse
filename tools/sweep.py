...
relief card and reports any that made no difference at all.

    python3 tools/sweep.py [--binary build/rptest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The pendulum's controls need a swinging lamp.** With no kick the cord's
length, its damping and the swing gain all provably do nothing. `rptest
--out` feeds the card's own spectrum -- a kick on the bass bins every half
second, a little hiss above -- through the host's element setter, 40 frames
at 60 fps, so the lamp is in motion on the frame that is read back.

**Kick is an event.** Its two ends are "never pressed" and "pressed before
the first frame"; the second sets the lamp swinging, which the 40 frames
then show.

**Most controls need the defaults' context**: Shadows only show where a
ridge casts one, Patina only where there is a recess, Roughness only where
there is a highlight. The card has all three, on purpose; a new control that
needs something else must say so in CONTEXT.

**An option's range is its element count.** An option parameter's range
reads back 0..1 whatever its element count, so `rptest --list` prints the
count as the range. The ends swept are those.
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "rptest"))
    ...
    scratch = tempfile.mkdtemp(prefix="rpsweep")

    # One render first: a runner with no GL at all cannot sweep anything, and
    # that has to read as a skip, loudly, not as twenty dead controls.
